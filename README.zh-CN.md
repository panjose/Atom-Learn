...
Core `0.14.0` 保留只读兼容性与确定性迁移规划，并加入下文所述的修复门禁。`atomlearn migrate status|plan|validate` 不会应用迁移；仅查看状态也不会创建平台用户数据目录。详见 [Core 版本与迁移](atom-learn/references/MIGRATIONS.md)。
