...
The normal first-use path is the resumable `start` wizard. Topic-only users can provide one phrase; for source, outline, and mixed requests, the harness translates the learner's request into the public start schema. Core derives a revisioned Goal Contract and explicit Corpus Policy, then returns revision-bound typed actions for clarification, local-candidate coverage judgment, policy-allowed Web Search, planning, phase confirmation, and first-Atom activation. The learner never edits intermediate YAML, interrupted runs replay the exact current action, and stale submissions cannot mutate newer state.
