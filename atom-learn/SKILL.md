...
6. Retrieve and deterministically rerank every Goal Contract anchor for every intake mode. Keep intake in `discovering` until harness verdicts over requirement-specific candidates pass coverage for the current intake, Goal Contract, and RAG revisions. Judge local candidates first. Use `rag correct` for weak or missing evidence only when Corpus Policy permits expansion; `closed_corpus` reports gaps and never uses Web Search.
...
8. For topic-only ambiguity, resolve only starting point, target depth, and use case. Offer `start_from_basics`, `map_first`, or `use_defaults`; otherwise use one 2–5 item adaptive diagnostic. A skipped or don't-know item stays `unknown`, raw responses are not persisted, and this pre-Atom diagnostic never creates mastery Evidence. Follow `assets/schemas/topic-diagnostic.schema.json`.
9. Ask only questions that materially change the path. Continue with recorded assumptions when uncertainty is non-blocking.
10. Build and import a source-grounded plan, then run `intake complete`, `validate`, and `render`.
...
python <SKILL_DIR>/scripts/atomlearn.py rag benchmark <benchmark-workspace> --profile core-release-v2
...
Run `start` with `--json` for harness work. Execute only its current `workflow_action`, then return an action-bound typed submission; never ask the learner to edit the intermediate file. For `diagnose_topic`, show the three one-click paths first; if adaptive, ask at most five items and submit only the privacy-minimized diagnostic contract. For `judge_coverage`, evaluate only the returned candidates. When it requests Web Search, return bounded `web_evidence` and candidate-grounded `verdicts`; never search or submit Web evidence for `closed_corpus`. When it reports a corpus gap, show the learner the missing anchors and ask whether to narrow the goal, add approved material, or explicitly change policy. When it requests a plan, return the source-grounded DAG. Show the phase preview before submitting confirmation, then show the proposed first Atom before confirming activation. A no-payload resume must replay the exact action without revision change; an old submission must fail stale. The auto-generated `.atomlearn/start.yaml` is orchestration state, not another user-authored form.
...
- Read [references/RAG.md](references/RAG.md) when indexing materials, retrieving course evidence, judging any intake's Goal Contract, applying Corpus Policy, correcting permitted gaps with Web Search, reranking, or evaluating retrieval and grounding quality.
