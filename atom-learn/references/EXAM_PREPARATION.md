...
`process-source` is the preferred path for an indexed paper: it consumes the active shared Document IR, preserves owning block IDs in each question locator, and does not create another full-text copy. `process` remains available when question, answer, and marking artifacts must be supplied together. Both paths automatically split stable question numbers and derive locators; `process` jointly scores the question stem, answer, and rubric when proposing an Atom mapping.

`semantic_mapping` in `assets/templates/exam-process.yaml` is `auto`, `required`, or `off`. Lexical evidence is deterministic and always available. Semantic evidence is admitted only when the RAG state has both a learned/provider embedding profile and a current benchmark-approved reranker. Approved results must resolve through the active source revision, Document IR block, and locator to source-bound Atom evidence. `auto` records a typed fallback when this gate is unavailable; `required` fails before mutating exam state; `off` makes the lexical choice explicit. Candidate output preserves the mapping method, lexical/semantic score decomposition, supporting locators, opposing candidates, source/RAG revision, runtime profile, and benchmark identity.

Every automatic mapping remains `pending` even when its score is high. Only confirmed or corrected mappings enter course coverage and planning. Inspect diagnostics and the review queue. Use the same knowledge-point ID, label, and Atom mapping throughout the corpus. Do not force a weak mapping to obtain 100% coverage. Preserve it as an explicit coverage gap.
...
The runtime calls this weighted estimate `structural_complexity` and reports a band from `foundation` through `challenge`. It is not presented as observed difficulty. Record a reviewed publisher/examiner rating with `exam record-official --input assets/templates/exam-official-difficulty.yaml`; an official level without its source and exact locator remains visible but is downgraded to structural evidence. Run `exam calibrate` only when qualified official anchors are available. The learned offset reports before/after MAE and leaves the anchors unchanged.

Use `exam record-empirical --input assets/templates/exam-empirical-difficulty.yaml` for aggregate performance. Preserve attempts, correct rate, named population, complete observation window, optional discrimination/time/IRT, source, and source locator. A record with fewer than 30 attempts or missing provenance remains visible but cannot become the effective difficulty. Effective priority is qualified empirical data, then qualified official difficulty, then structural complexity, with the selected basis, qualification reasons, and uncertainty exposed.
...
For a one-time calendar preview, use `exam daily-plan --input assets/templates/exam-daily-plan.yaml`. Persist a canonical calendar with `exam replan ... --reason initial --expected-schedule-revision 0`, then inspect it with `exam plan-status`. Canonical scheduling requires an exam target date, and the payload target must match it; use `exam set-target` first instead of creating two conflicting dates. The schedule has an independent revision and binds the exact exam and course revisions. It schedules prerequisite, learn, remediate, review, and practice work within declared weekdays and minutes, reserving the final review window. If work does not fit, it persists an `infeasible` revision with the remaining tasks, minute gap, and adjustment options without lowering mastery.

Submit one `assets/templates/exam-day-outcome.yaml` with `exam record-day` for every planned date. A completed day must name every planned task; a partial day names only completed tasks; a missed day names none. Status derives `due` and `overdue` events and invalidates the plan after missed, partial, availability-changed, or unrecorded past days. It also invalidates after new mastery/retention Evidence, a target changed through `exam set-target`, changed corpus, completed mapping or difficulty review, revoked skips, inserted prerequisites, or course-plan changes. Replan with the exact schedule revision and a matching reason. Completion may carry forward only across time/capacity replans; content or Evidence changes force a fresh queue. Reminder integrations are external adapters over these outputs, not a requirement for the CLI workflow.
