...
The evaluator reports mean `recall_at_k`, MRR, nDCG@k, citation correctness, and unsupported-claim rate, plus per-query and per-claim diagnostics. With no thresholds or named profile it returns `quality_gate: report_only`. An ad hoc pass/fail gate requires all five thresholds; partial threshold sets are rejected so omitted dimensions cannot inherit permissive defaults. Stable gates use `rag benchmark <fresh-workspace> --profile core-release-v2`. Its read-only held-out release set contains seven separately versioned profiles: `lexical-baseline`, `true-cross-lingual`, `domain-shift`, `hard-negatives`, `structured-docs`, `ocr-layout`, and `grounding-adversarial`. Cross-lingual relevant blocks are monolingual and exclude the query language; hard negatives cover condition, negation, direction, and synonym traps. HTML, DOCX, PDF formula/table, and OCR fixtures pass through the production parser and the same retrieval/grounding evaluation.

The release report includes percentile-bootstrap confidence intervals, per-profile gates, parser regression, adversarial grounding detection, and a failure taxonomy that distinguishes retrieval, reranking, locator, and generation-grounding failures. Dataset, parser, embedding, reranker, runtime, seed, split, and training prohibition are fixed in the profile. The default hash path is explicitly an honest lexical/hash-hybrid baseline rather than learned semantics. A learned profile is not called stable merely because it beats the baseline; it still needs the shipped runtime distribution and this unchanged release set. The benchmark is an engineering claim about retrieval and grounding, never evidence of learning benefit. A named profile and explicit thresholds cannot be combined. Use active chunk IDs as retrieval relevance and claim-support labels; rerun or regenerate an ad hoc benchmark when source revisions change. Start from `assets/templates/rag-evaluation.yaml`.
