...
Ad hoc evaluation without thresholds is explicitly `report_only`; five core thresholds are required for an ad hoc gate. Stable gates use the read-only held-out `core-release-v2` suite. Its seven named/versioned profiles cover an honest lexical/hash baseline, true cross-lingual retrieval without relevant-block language leakage, domain shift, condition/negation/direction/synonym hard negatives, structured documents, OCR/layout, and adversarial grounding. It runs real HTML, DOCX, PDF, and OCR ingestion; gates source diversity, freshness, correction success, and residual gaps; reports bootstrap uncertainty; and distinguishes retrieval, reranking, locator, and generation-grounding failure. The fixed dataset/parser/model/runtime identities make results comparable, but the suite establishes only retrieval and grounding engineering performance, not learning benefit or blanket learned-semantic stability. Add failure queries whenever learners expose a synonym, language, document-structure, OCR, table, formula, or global-context retrieval miss.
